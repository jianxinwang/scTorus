###################################################################################################
# Loop through each dataset and make trajectory plots, ect
###################################################################################################
...
run_scTorus <- function(x, genes.to.highlight = NULL, 
    ...
    if (method == 'lmds'){
        ...
        if (dim(x.scaled)[1] > 10000){
            to.keep <- sample(1:nrow(x), 10000, replace = FALSE)
    ...
    colnames(dd) <- c('Time', 'Gene', 'Z-Score of Expression')
    
    # make lines to have similar colors for each gene
    pt <- ggplot(dd) + geom_line(aes(x=Time, y=`Relative Expression`, colour=Gene, linetype=Gene)) + labs(title=NULL) +
